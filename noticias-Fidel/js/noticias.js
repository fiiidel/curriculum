...
function resaltarNoticiasQueContengan(palabra){
  ...
  for(var contador = 0; contador<noticias.length; contador++){
    noticiaActual = noticias[contador];
    palabradevuelta=contienePalabra(noticiaActual,palabra);
    if (palabradevuelta==true) {
...
function resaltarNoticiasQueContengan2(palabra){
  ...
  for(var contador = 0; contador<noticias.length; contador++){
    ...
    if (palabradevuelta==true) {
      ...
      cambiarColor(noticiaActual, 'rgb(0, 270, 0)');
    }
...
function eliminarnoticiasdegoogle(palabra)
{
  for(var contador = 0; contador<noticias.length; contador++){
    noticiaActual = noticias[contador];
    sacarnoticia=contienePalabra(noticiaActual,palabra);
    if (sacarnoticia==true) {
...
resaltarNoticiasQueContengan2("ARGENTINA");
eliminarnoticiasdegoogle("Google");
recortarNoticias(20);
